...
void codegen(char * name){
    ...
    for(int i=0;i<count;i++){
       ...
       if(!strcasecmp(curr.op,"=")){
        ...
        if(lval->is_array==0){
            ...
            printf("PRINT ME GHUSA RAHA HAI\n");
    ...
    //    else if(!strcasecmp(curr.op,"PLUS") ||!strcasecmp(curr.op,"MINUS") || !strcasecmp(curr.op,"MUL") || !strcasecmp(curr.op,"DIV")){
    ...
    //             // if(rightOp.arg.arg_var->is_array == 1)
    //             //     fprintf(fp,"\t\t\t mov bx,[rbp-%d] ;moving %s from memory to rbx \n",t->offset,t->id_lexeme);
    //             // else
    //                 fprintf(fp,"\t\t\t mov bx,[rbp-%d] ;moving %s from memory to rbx \n",curr.index2.arg.arg_num,t->id_lexeme);
    //         }
